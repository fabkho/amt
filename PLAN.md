...
**Quellenliste (Design-Entscheidung 20.08.):** Die Crawl-Liste wird **vom Benutzer selbst gepflegt und startet leer** (nur Arbeitnow als Zero-Config-Board-Default). Weil das Tool sie hydratet, lebt sie NICHT in der `profile.config.ts` (die ist ausschließlich handeditiert), sondern in einer tool-verwalteten `sources.yaml` in `JOB_KIT_HOME`. Erster `crawl` mit leerer Liste: im TTY interaktiver Dialog („Welche Firmen?") → automatisches ATS-Probing (5 API-Muster + Careers-Seiten-Grep) → Slugs in `sources.yaml`; im Agent-Kontext strukturierter Hinweis, der Agent fragt im Chat und ruft `add_source` auf. `sources add <firma>` macht die Discovery immer mit — niemand rät Slugs. Das `sources`-Feld im Profil-Schema entfällt entsprechend.
