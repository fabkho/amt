...
**Setup:** `pnpm add -g git+ssh://git@github.com/fabkho/amt.git`, then `amt doctor` (installs Chromium for PDFs) and `amt init` (profile + sources). Copy this directory to `~/.pi/agent/extensions/amt/` and `/reload`.
...
Agent channels (LinkedIn guest, StepStone) live in `~/.config/amt/sources.yaml` under `channels` — execute those yourself and feed findings via `["import", "<url>", "--company", "<name>", "--title", "<title>"]` (ATS URLs need no manual fields).
