...
| Tooling | pnpm, tsdown, vitest, **oxlint** statt eslint (~700 Core-Regeln; die stdout-Disziplin aus dem i18n-kit = `no-console` global + Override für die Logger-Datei — Core-Regel, kein Plugin nötig; type-aware optional via oxlint-tsgolint, `tsc`-Typecheck bleibt eigener Schritt), `tsconfig.base` mit strict-Flags, husky + commitlint, release-please (node), **fallow** (Dead-Code-/Komplexitäts-Audit; `.fallowrc.json` mit `gate: "new-only"` + committete Baselines wie im i18n-kit — bei Greenfield starten die Baselines leer, das Gate greift ab Commit 1 voll) | i18n-kit-Muster + oxlint-Recherche 08/2026 |
| Vite+ | **Jetzt nicht** — seit 10/2025 vollständig MIT-open-source und genau unser Tool-Bündel (Vitest/Oxlint/Oxfmt/tsdown) unter einem CLI, aber Stand 08/2026 Alpha. Später Sammel-Umstieg möglich, da identische Unterbau-Tools/Configs — kein Lock-in in beide Richtungen | voidzero.dev-Announcements, heise |
...
CLI (citty, `createCommand`-Factory): `init` · `doctor` (Chromium/Profil-Check, installiert Playwright-Browser bei Bedarf) · `crawl` · `import <url>` · `list` · `status <slug> <st>` · `show <slug>` · `apply <slug>` (Ordner + Rendern) · `render-cv` / `render-letter` · `views`. Konventionen: Exit-Codes 0/1/2, stdout/stderr-Trennung, `--json`, Dry-run-Defaults.
